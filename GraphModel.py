...
class FourierFeatures(nn.Module):
    ...
    def forward(self, indices: torch.Tensor) -> torch.Tensor:
        ...
        return self.precomputed_features[idx_long]  # (B, L, 2*K)


class StateEncoder(nn.Module):
    """Encodes Tower of Hanoi state into node embeddings with padding support."""
    
    def __init__(self, N_max: int, d_model: int, K_fourier: int = 8):
        super().__init__()
        self.N_max = N_max
        ...
        # Type embeddings
        self.disk_type = nn.Parameter(torch.tensor([1.0, 0.0]))
        self.rod_type = nn.Parameter(torch.tensor([0.0, 1.0]))
        ...
        # Projection layers
        disk_feature_dim = 2 + 2*K_fourier + 3 + 3  # type + fourier + position_info + rod_onehot
        rod_feature_dim = 2 + 1 + 1 + 2*2*K_fourier  # type + len_norm + is_empty + top/bottom fourier
    ...
    def forward(self, state: torch.Tensor, lengths: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, Dict]:
        ...
        for i in range(max_N):
            ...
            # Rod information (which rod this disk is on)
            rod_indices = state[:, i].clamp(min=0).long()  # Clamp -1 to 0 temporarily
            rod_onehot = self.rod_embeddings[rod_indices]  # (B, 3)
            
            # Concatenate all disk features
            disk_features = torch.cat([disk_type, fourier_size, position_info, rod_onehot], dim=1)
        ...
        for k in range(3):
            ...
            # Top disk Fourier features
            top_disk_indices = state_info['top_disk'][:, k].float()  # (B,)
            # Use NULL vector for empty rods
            empty_mask = state_info['is_empty'][:, k]  # (B,)
            # Avoid passing -1 to FourierFeatures (strict cached lookup)
            top_indices_safe = torch.clamp(top_disk_indices, min=0)
            top_fourier = self.fourier_features(top_indices_safe)  # (B, 2*K)
            top_fourier = torch.where(empty_mask.unsqueeze(1), self.null_fourier.unsqueeze(0).expand(batch_size, -1), top_fourier)
            
            # Bottom disk Fourier features  
            bottom_disk_indices = state_info['bottom_disk'][:, k].float()  # (B,)
            # Avoid passing -1 to FourierFeatures (strict cached lookup)
            bottom_indices_safe = torch.clamp(bottom_disk_indices, min=0)
            bottom_fourier = self.fourier_features(bottom_indices_safe)  # (B, 2*K)
            bottom_fourier = torch.where(empty_mask.unsqueeze(1), self.null_fourier.unsqueeze(0).expand(batch_size, -1), bottom_fourier)
            
            # Concatenate all rod features
            rod_features = torch.cat([rod_type, len_norm, is_empty, top_fourier, bottom_fourier], dim=1)
            rod_embedding = self.rod_projection(rod_features)  # (B, d_model)
            embeddings.append(rod_embedding)
        
        # Stack all embeddings
        embeddings = torch.stack(embeddings, dim=1)  # (B, max_N+3, d_model)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, N: int, state_info: Optional[Dict] = None) -> torch.Tensor:
        ...
        if state_info is not None and 'padding_mask' in state_info:
            ...
            # Extend mask to include rod nodes (which are never padded)
            rod_mask = torch.ones(batch_size, 3, dtype=torch.bool, device=padding_mask.device)
            full_mask = torch.cat([padding_mask, rod_mask], dim=1)  # (B, max_N+3)
        ...
        for b in range(batch_size):
            for i in range(seq_len):
                for j in range(seq_len):
                    ...
                    if is_i_disk and is_j_disk:
                        rod_i = state_info['state'][b, i]
                        rod_j = state_info['state'][b, j]
                        if rod_i == rod_j:
...
class ReadoutHead(nn.Module):
    """Readout head that produces action probabilities from rod embeddings."""
    
    def __init__(self, d_model: int):
        super().__init__()
        self.W = nn.Linear(d_model, d_model, bias=False)
        self.u = nn.Linear(2 * d_model, 1)
        self.c = nn.Parameter(torch.zeros(1))
    
    def forward(self, embeddings: torch.Tensor, N: int, state_info: Dict) -> torch.Tensor:
        """
        Args:
            embeddings: (batch_size, N+3, d_model)
            N: number of disks
            state_info: state information for legality masking
        Returns:
            logits: (batch_size, 3, 3) logits for from->to moves
        """
        batch_size = embeddings.shape[0]
        
        # Extract rod embeddings (last 3 positions)
        rod_embeddings = embeddings[:, N:N+3, :]  # (B, 3, d_model)
        
        # Compute pairwise logits
        logits = torch.zeros(batch_size, 3, 3, device=embeddings.device)
        
        for a in range(3):
            for b in range(3):
                if a != b:
                    h_a = rod_embeddings[:, a, :]  # (B, d_model)
                    h_b = rod_embeddings[:, b, :]  # (B, d_model)
                    
                    # Bilinear term: h_a^T W h_b
                    bilinear = torch.sum(h_a * torch.matmul(h_b, self.W.weight), dim=1)  # (B,)
                    
                    # Linear term: u^T [h_a || h_b]
                    concat = torch.cat([h_a, h_b], dim=1)  # (B, 2*d_model)
                    linear = self.u(concat).squeeze(-1)  # (B,)
                    
                    logits[:, a, b] = bilinear + linear + self.c
        
        # Apply legality mask
        masked_logits = self._apply_legality_mask(logits, state_info)
        
        return masked_logits
    
    def _apply_legality_mask(self, logits: torch.Tensor, state_info: Dict) -> torch.Tensor:
        ...
        for b in range(batch_size):
            for a_rod in range(3):
                for b_rod in range(3):
                    # Check if move is legal
                    is_legal = self._is_move_legal(state_info, b, a_rod, b_rod)
                    if not is_legal:
                        masked_logits[b, a_rod, b_rod] = float('-inf')
...
class HanoiGraphTransformer(nn.Module):
    """Complete Graph Transformer for Tower of Hanoi with padding support."""
    
    def __init__(self, N_max: int, d_model: int = 128, n_heads: int = 8, 
        ...
        # Components
        self.encoder = StateEncoder(N_max, d_model, K_fourier)
    ...
    def forward(self, state: torch.Tensor, lengths: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        Args:
            state: (batch_size, max_N) where state[b,i] = rod of disk i, or -1 for padding
            lengths: (batch_size,) actual number of disks for each sample (optional, inferred if not provided)
        Returns:
            logits: (batch_size, 3, 3) action logits
        """
        batch_size, max_N = state.shape
    ...
    def get_action_probs(self, state: torch.Tensor, lengths: Optional[torch.Tensor] = None) -> torch.Tensor:
        """Get action probabilities (softmax over legal moves)."""
        logits = self.forward(state, lengths)
        batch_size = logits.shape[0]
        flat_logits = logits.view(batch_size, -1)
        probs = F.softmax(flat_logits, dim=-1)
        return probs.view(batch_size, 3, 3)
...
if __name__ == "__main__":
    ...
    N = 4
    batch_size = 32
    model = HanoiGraphTransformer(N_max=10, d_model=64, n_heads=4, n_layers=3, use_structural_bias=True)

    for batch in dataloader:
        print(f"Batch states shape: {batch['state'].shape}")
        print(f"Batch targets shape: {batch['noisy_target'].shape}")
        state_example = batch['state']
        lengths_example = batch['lengths']
        output = model(state_example, lengths_example)
        print(f"Model output shape: {output.shape}")
        break  # Just show one batch
    
    states_same_length = torch.randint(0, 3, (batch_size, N))
    print(f"Same-length input shape: {states_same_length.shape}")
    
    logits_same = model(states_same_length)
    print(f"Same-length output shape: {logits_same.shape}")
    
    # Test with variable-length sequences
    print(f"\n=== Testing Variable Length Sequences ===")
    states_variable = [
        torch.randint(0, 3, (3,)),  # 3 disks
        torch.randint(0, 3, (5,)),  # 5 disks  
        torch.randint(0, 3, (4,)),  # 4 disks
        torch.randint(0, 3, (6,))   # 6 disks
    ]
    
    print("Variable-length inputs:")
    for i, state in enumerate(states_variable):
        print(f"  Sample {i}: {state.shape} - {state.tolist()}")
    
    logits_variable = model(states_variable)
    print(f"Variable-length output shape: {logits_variable.shape}")
    
    # Test without structural bias
    print(f"\n=== Testing Without Structural Bias ===")
    model_no_bias = HanoiGraphTransformer(N_max=10, d_model=64, n_heads=4, n_layers=3, use_structural_bias=False)
    
    logits_no_bias = model_no_bias(states_same_length)
    print(f"No-bias output shape: {logits_no_bias.shape}")
    
    # Test variable length with no bias
    logits_variable_no_bias = model_no_bias(states_variable)
    print(f"Variable-length no-bias output shape: {logits_variable_no_bias.shape}")
    
    # Test with dataset integration
    print(f"\n=== Testing Dataset Integration ===")
    
    # Generate mixed-size dataset
    datasets = []
    for N in [3, 4, 5]:
        from HanoiDataset import HanoiDatasetGenerator
        generator = HanoiDatasetGenerator(N, noise_rate=0.1)
        samples = generator.generate_dataset(num_samples=5)
        datasets.extend(samples)
    
    print(f"Generated {len(datasets)} samples with mixed sizes")
    
    # Test forward pass on mixed dataset
    sample_states = [torch.tensor(d['state'], dtype=torch.long) for d in datasets[:8]]
    sample_targets = [torch.tensor(d['noisy_target'], dtype=torch.float32) for d in datasets[:8]]
    
    print("Mixed dataset sample shapes:")
    for i, (state, target) in enumerate(zip(sample_states, sample_targets)):
        print(f"  Sample {i}: state {state.shape}, target {target.shape}")
    
    # Forward pass on mixed data
    mixed_logits = model(sample_states)
    print(f"Mixed dataset output shape: {mixed_logits.shape}")
    
    # Show that each sample gets 3x3 output regardless of input size
    for i in range(min(3, len(sample_states))):
        print(f"Sample {i} (N={len(sample_states[i])}): output shape {mixed_logits[i].shape}")