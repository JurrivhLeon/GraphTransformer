...
# Import our modules
from NAGT import HanoiGraphTransformer
from HanoiDataset import HanoiDatasetGenerator, HanoiDataset, TowerOfHanoiState, hanoi_collate_fn
...
class HanoiTrainer:
    ...
    def compute_loss(self, logits: torch.Tensor, targets: torch.Tensor) -> Tuple[torch.Tensor, Dict]:
        """Compute training loss with optional entropy regularization."""
        batch_size = logits.shape[0]
        
        # Flatten logits and targets for cross-entropy
        flat_logits = logits.view(batch_size, -1)  # (B, 9)
        flat_targets = targets.view(batch_size, -1)  # (B, 9)
        
        # Masked cross-entropy loss
        # Only compute loss on legal moves (non-zero targets)
        legal_mask = flat_targets > 0
        
        # Standard cross-entropy
        flat_logits = torch.nan_to_num(flat_logits, neginf=-1e9)
        log_probs = F.log_softmax(flat_logits, dim=-1)
        loss = -torch.sum(flat_targets * log_probs, dim=-1)  # (B,)
        
        # Entropy regularization (encourage exploration)
        entropy_loss = 0.0
        if self.entropy_weight > 0:
            probs = F.softmax(flat_logits, dim=-1)
            # Only compute entropy on legal moves
            masked_probs = probs * legal_mask.float()
            masked_probs = masked_probs / (masked_probs.sum(dim=-1, keepdim=True) + 1e-8)
            entropy = -torch.sum(masked_probs * torch.log(masked_probs + 1e-8), dim=-1)
        ...
        with torch.no_grad():
            pred_actions = torch.argmax(flat_logits, dim=-1)  # (B,)
            true_actions = torch.argmax(flat_targets, dim=-1)  # (B,)
            accuracy = (pred_actions == true_actions).float().mean()
    ...
    def train_step(self, batch: Union[Dict, List]) -> Dict:
        ...
        # Forward pass
        logits = self.model(states, lengths)
            
        # Compute loss
        total_loss, total_metrics = self.compute_loss(logits, targets)
        total_loss.backward()
...
def solve_puzzle_with_model(model: HanoiGraphTransformer, 
