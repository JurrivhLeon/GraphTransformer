...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, dropout: float = 0.1, use_structural_bias: bool = False):
        super().__init__()
...
class ReadoutHead(nn.Module):
    """Readout head that produces action logits from rod embeddings using MLP."""
    ...
    def forward(self, embeddings: torch.Tensor, N: int, state_info: Dict) -> torch.Tensor:
        ...
        # Extract rod embeddings (first 3 positions due to rods-first)
        rod_embeddings = embeddings[:, 0:3, :]  # (B, 3, d_model)
        ...
        for a in range(3):
            for b in range(3):
                if a != b:
                    h_a = rod_embeddings[:, a, :]
                    h_b = rod_embeddings[:, b, :]
                    pair = torch.cat([h_a, h_b], dim=1)  # (B, 2*d_model)
                    logits[:, a, b] = self.mlp_pair(pair).squeeze(-1)
        ...
        for b in range(batch_size):
            for a_rod in range(3):
                for b_rod in range(3):
                    ...
                    if not is_legal:
                        masked_logits[b, a_rod, b_rod] = LARGE_NEG
